const admin = require('../firebase-config'); // Mengimpor konfigurasi Firebase untuk mengakses Firestore
const { generateId } = require('../services/generateId'); // Mengimpor fungsi generateId untuk membuat ID unik

// Fungsi utama yang menerima parameter app (instance Express)
const level = (app) => {

  // Endpoint GET untuk mengambil semua data level dan soal terkait
  app.get('/list-level', async (req, res) => {
    try {
      const db = admin.firestore(); // Inisialisasi Firestore dari admin Firebase

      // Mendapatkan semua dokumen dari koleksi 'Levels'
      const levelsSnapshot = await db.collection('Levels').get();

      // Jika tidak ada level ditemukan, kembalikan status 404 (Not Found)
